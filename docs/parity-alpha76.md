...
| `managed.py` | `managed*.go`, `service*.go`, `tokenizer.go`, `base_only.go`, `lazy_rest.go`, `future_facade.go` | partial | Config normalization, trainer/deployment provisioning orchestration, reattach, sampler/weight-sync attachment, separate reference provisioning, reference job IDs/release, optional and required service metadata accessors, metadata resolution, cleanup planning, tokenizer/base-only helpers, lazy REST and server-capabilities responses, public service/training-client facades, managed lifecycle futures, and opt-in live contract tests exist. Needs live execution against Fireworks. |
