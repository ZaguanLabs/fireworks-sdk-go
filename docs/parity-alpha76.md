...
| `sampling.py` | `sampling.go`, `service_client.go`, `future_facade.go` | partial | The FireTitan sampler, output shape helpers, concurrent `n` sampling, facade sampler hooks, and future wrappers are covered. |
