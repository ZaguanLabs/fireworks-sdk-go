...
This project targets Fireworks Python SDK `1.2.0-alpha.78` and keeps the Go
package version aligned exactly:

```go
fireworks.Version // "1.2.0-alpha.78"
```
...
[`docs/resource-type-parity-alpha78.md`](docs/resource-type-parity-alpha78.md).
The broader training parity matrix is in
[`docs/parity-alpha78.md`](docs/parity-alpha78.md).
...
```sh
FIREWORKS_PY_TYPES_ROOT=/path/to/python-sdk-1.2.0-alpha.78/src/fireworks/types \
  go generate ./types
```
...
```text
docs/fireworks-py/python-sdk-1.2.0-alpha.78/src/fireworks/types
```
...
```sh
git tag v1.2.0-alpha.78
git push origin v1.2.0-alpha.78
```
...
- tags with a prerelease suffix, such as `v1.2.0-alpha.78`, create a
